...
K8S_LOCAL_CATALOG_IMAGE ?= sretail-catalog:dev
...
.PHONY: local-config local-build local-up local-test local-verify local-logs local-down \
	k8s-local-config k8s-local-render k8s-local-build k8s-local-load k8s-local-deploy \
	k8s-local-status
...
k8s-local-load: k8s-local-config k8s-local-build
